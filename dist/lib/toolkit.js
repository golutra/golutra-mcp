...
export function registerTools(server, contextStore, gateway) {
        ...
        try {
            ...
            const result = await gateway.inviteTerminals(runtimeContext, {
                ...
                ...(input.memberOnboardingPromptModeTemplates !== undefined
                    ? {
                        memberOnboardingPromptModeTemplates: input.memberOnboardingPromptModeTemplates
                    }
                    : {}),
                ...(input.memberDispatchMessagePrefixRules !== undefined
                    ? {
                        memberDispatchMessagePrefixRules: input.memberDispatchMessagePrefixRules
                    }
                    : {})
            });
    ...
    server.registerTool("golutra-store-command", {
        ...
        try {
            const runtimeContext = contextStore.resolveCommandContext(input);
            const result = await gateway.executeStoreCommand(runtimeContext, input.commandType, input.payload);
